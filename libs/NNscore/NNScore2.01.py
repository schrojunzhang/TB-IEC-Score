# NNScore 2.01 is released under the GNU General Public License (see http://www.gnu.org/licenses/gpl.html).
# If you have any questions, comments, or suggestions, please don't hesitate to contact me, Jacob
# Durrant, at jdurrant [at] ucsd [dot] edu. If you use NNScore 2.01 in your work, please cite [REFERENCE HERE].


from libs.NNScore.network_data import networks
import textwrap
import math
import os
import sys
...
vina_executable = f'{project_dir}/libs/Smina/vina'
...
class ffnet:
    ...
    def setin(self):
        ...
        for k in range(1, len(self.inno) + 1):
            ...
            self.units[self.inno[k]] = self.eni[k][1] * \
                self.input[k-1] + self.eni[k][2]
            ...
            for xn in range(1, len(self.conec) + 1):
                ...
                if trg != ctrg:
                    ...
                    if src == 0:  # handle bias
                        ...
                        self.units[ctrg] = self.units[ctrg] + \
                            self.units[src] * self.weights[xn]
            self.units[ctrg] = 1.0 / \
                (1.0+math.exp(-self.units[ctrg]))  # for last unit

    def getout(self):
        # get and denormalize output units

        for k in range(1, len(self.outno)+1):
            self.output[k] = self.deo[k][1] * \
                self.units[self.outno[k]] + self.deo[k][2]
...
class atom:
    ...
    def CreatePDBLine(self, index):
        ...
        output = "ATOM "
        output = output + str(index).rjust(6) + \
            self.atomname.rjust(5) + self.residue.rjust(4)
    ...
    def ReadPDBLine(self, Line):
        ...
        if len(self.atomname) == 1:
            ...
            # This line is necessary for babel to work, though many PDBs in the PDB would have this line commented out
            self.atomname = self.atomname + " "
...
class PDB:
    ...
    def __init__(self):
        ...
        self.protein_resnames = ["ALA", "ARG", "ASN", "ASP", "ASH", "ASX", "CYS", "CYM", "CYX", "GLN", "GLU", "GLH", "GLX",
                                 "GLY", "HIS", "HID", "HIE", "HIP", "ILE", "LEU", "LYS", "LYN", "MET", "PHE", "PRO", "SER", "THR", "TRP", "TYR", "VAL"]
        self.aromatic_rings = []
    ...
    def LoadPDB_from_list(self, lines, line_header=""):
        ...
        for t in range(0, len(lines)):
            ...
            if len(line) >= 7:
                ...
                if line[0:4] == "ATOM" or line[0:6] == "HETATM":
                    ...
                    key = TempAtom.atomname.strip() + "_" + str(TempAtom.resid) + "_" + TempAtom.residue.strip() + \
                        "_" + TempAtom.chain.strip()  # this string unique identifies each atom
        ...
        # only for the ligand, because bonds can be inferred based on atomnames from PDB
    ...
    def SavePDBString(self):
        ...
        for atomindex in self.AllAtoms:
            ToOutput = ToOutput + \
                self.AllAtoms[atomindex].CreatePDBLine(atomindex) + "\n"
    ...
    def CheckProteinFormat_process_residue(self, residue, last_key):
        ...
        if real_resname in self.protein_resnames:  # so it's a protein residue
            ...
            if not "N" in residue:
                self.printout('WARNING: There is no atom named "N" in the protein residue ' + last_key +
                ...
                print("")
            if not "C" in residue:
                self.printout('WARNING: There is no atom named "C" in the protein residue ' + last_key +
                              '. Please use standard naming conventions for all protein residues. This atom is needed to determine secondary structure. If this residue is far from the active site, this warning may not affect the NNScore.')
                print("")
            if not "CA" in residue:
                self.printout('WARNING: There is no atom named "CA" in the protein residue ' + last_key +
                              '. Please use standard naming conventions for all protein residues. This atom is needed to determine secondary structure. If this residue is far from the active site, this warning may not affect the NNScore.')
                print("")

            if real_resname == "GLU" or real_resname == "GLH" or real_resname == "GLX":
                if not "OE1" in residue:
                    self.printout('WARNING: There is no atom named "OE1" in the protein residue ' + last_key +
                ...
                if not "OE2" in residue:
                    self.printout('WARNING: There is no atom named "OE2" in the protein residue ' + last_key +
                                  '. Please use standard naming conventions for all protein residues. This atom is needed to determine salt-bridge interactions. If this residue is far from the active site, this warning may not affect the NNScore.')
                    print("")

            if real_resname == "ASP" or real_resname == "ASH" or real_resname == "ASX":
                if not "OD1" in residue:
                    self.printout('WARNING: There is no atom named "OD1" in the protein residue ' + last_key +
                                  '. Please use standard naming conventions for all protein residues. This atom is needed to determine salt-bridge interactions. If this residue is far from the active site, this warning may not affect the NNScore.')
                    print("")
                if not "OD2" in residue:
                    self.printout('WARNING: There is no atom named "OD2" in the protein residue ' + last_key +
                                  '. Please use standard naming conventions for all protein residues. This atom is needed to determine salt-bridge interactions. If this residue is far from the active site, this warning may not affect the NNScore.')
                    print("")

            if real_resname == "LYS" or real_resname == "LYN":
                if not "NZ" in residue:
                    self.printout('WARNING: There is no atom named "NZ" in the protein residue ' + last_key +
            ...
            if real_resname == "ARG":
                if not "NH1" in residue:
                    self.printout('WARNING: There is no atom named "NH1" in the protein residue ' + last_key +
                                  '. Please use standard naming conventions for all protein residues. This atom is needed to determine pi-cation and salt-bridge interactions. If this residue is far from the active site, this warning may not affect the NNScore.')
                    print("")
                if not "NH2" in residue:
                    self.printout('WARNING: There is no atom named "NH2" in the protein residue ' + last_key +
                                  '. Please use standard naming conventions for all protein residues. This atom is needed to determine pi-cation and salt-bridge interactions. If this residue is far from the active site, this warning may not affect the NNScore.')
                    print("")

            if real_resname == "HIS" or real_resname == "HID" or real_resname == "HIE" or real_resname == "HIP":
                if not "NE2" in residue:
                    self.printout('WARNING: There is no atom named "NE2" in the protein residue ' + last_key +
                                  '. Please use standard naming conventions for all protein residues. This atom is needed to determine pi-cation and salt-bridge interactions. If this residue is far from the active site, this warning may not affect the NNScore.')
                    print("")
                if not "ND1" in residue:
                    self.printout('WARNING: There is no atom named "ND1" in the protein residue ' + last_key +
                                  '. Please use standard naming conventions for all protein residues. This atom is needed to determine pi-cation and salt-bridge interactions. If this residue is far from the active site, this warning may not affect the NNScore.')
                    print("")

            if real_resname == "PHE":
                if not "CG" in residue:
                    self.printout('WARNING: There is no atom named "CG" in the protein residue ' + last_key +
                ...
                if not "CD1" in residue:
                    self.printout('WARNING: There is no atom named "CD1" in the protein residue ' + last_key +
                                  '. Please use standard naming conventions for all protein residues. This atom is needed to determine pi-pi and pi-cation interactions. If this residue is far from the active site, this warning may not affect the NNScore.')
                    print("")
                if not "CD2" in residue:
                    self.printout('WARNING: There is no atom named "CD2" in the protein residue ' + last_key +
                                  '. Please use standard naming conventions for all protein residues. This atom is needed to determine pi-pi and pi-cation interactions. If this residue is far from the active site, this warning may not affect the NNScore.')
                    print("")
                if not "CE1" in residue:
                    self.printout('WARNING: There is no atom named "CE1" in the protein residue ' + last_key +
                                  '. Please use standard naming conventions for all protein residues. This atom is needed to determine pi-pi and pi-cation interactions. If this residue is far from the active site, this warning may not affect the NNScore.')
                    print("")
                if not "CE2" in residue:
                    self.printout('WARNING: There is no atom named "CE2" in the protein residue ' + last_key +
                                  '. Please use standard naming conventions for all protein residues. This atom is needed to determine pi-pi and pi-cation interactions. If this residue is far from the active site, this warning may not affect the NNScore.')
                    print("")
                if not "CZ" in residue:
                    self.printout('WARNING: There is no atom named "CZ" in the protein residue ' + last_key +
                                  '. Please use standard naming conventions for all protein residues. This atom is needed to determine pi-pi and pi-cation interactions. If this residue is far from the active site, this warning may not affect the NNScore.')
                    print("")
            ...
            if real_resname == "TRP":
                ...
                if not "NE1" in residue:
                    self.printout('WARNING: There is no atom named "NE1" in the protein residue ' + last_key +
                                  '. Please use standard naming conventions for all protein residues. This atom is needed to determine pi-pi and pi-cation interactions. If this residue is far from the active site, this warning may not affect the NNScore.')
                    print("")
                if not "CE2" in residue:
                    self.printout('WARNING: There is no atom named "CE2" in the protein residue ' + last_key +
                ...
                if not "CE3" in residue:
                    self.printout('WARNING: There is no atom named "CE3" in the protein residue ' + last_key +
                                  '. Please use standard naming conventions for all protein residues. This atom is needed to determine pi-pi and pi-cation interactions. If this residue is far from the active site, this warning may not affect the NNScore.')
                    print("")
                if not "CZ2" in residue:
                    self.printout('WARNING: There is no atom named "CZ2" in the protein residue ' + last_key +
                                  '. Please use standard naming conventions for all protein residues. This atom is needed to determine pi-pi and pi-cation interactions. If this residue is far from the active site, this warning may not affect the NNScore.')
                    print("")
                if not "CZ3" in residue:
                    self.printout('WARNING: There is no atom named "CZ3" in the protein residue ' + last_key +
                                  '. Please use standard naming conventions for all protein residues. This atom is needed to determine pi-pi and pi-cation interactions. If this residue is far from the active site, this warning may not affect the NNScore.')
                    print("")
                if not "CH2" in residue:
                    self.printout('WARNING: There is no atom named "CH2" in the protein residue ' + last_key +
                                  '. Please use standard naming conventions for all protein residues. This atom is needed to determine pi-pi and pi-cation interactions. If this residue is far from the active site, this warning may not affect the NNScore.')
                    print("")
    ...
    def CreateBondsByDistance(self):
        for AtomIndex1 in self.NonProteinAtoms:
            ...
            if not atom1.residue[-3:] in self.protein_resnames:
    ...
    def BondLength(self, element1, element2):
        ...
        if element1 == "S" and element2 == "S":
            distance = 2.048
        if (element1 == "C" and element2 == "H") or (element1 == "H" and element2 == "C"):
            distance = 1.059
        if (element1 == "C" and element2 == "N") or (element1 == "N" and element2 == "C"):
            distance = 1.469
        if (element1 == "C" and element2 == "O") or (element1 == "O" and element2 == "C"):
            distance = 1.413
        if (element1 == "C" and element2 == "S") or (element1 == "S" and element2 == "C"):
            distance = 1.819
        if (element1 == "N" and element2 == "H") or (element1 == "H" and element2 == "N"):
            distance = 1.009
        if (element1 == "N" and element2 == "O") or (element1 == "O" and element2 == "N"):
            distance = 1.463
        if (element1 == "O" and element2 == "S") or (element1 == "S" and element2 == "O"):
            distance = 1.577
        if (element1 == "O" and element2 == "H") or (element1 == "H" and element2 == "O"):
            distance = 0.967
        if (element1 == "S" and element2 == "H") or (element1 == "H" and element2 == "S"):
            ...
            distance = 2.025/1.5
        if (element1 == "S" and element2 == "N") or (element1 == "N" and element2 == "S"):
            distance = 1.633

        if (element1 == "C" and element2 == "F") or (element1 == "F" and element2 == "C"):
            distance = 1.399
        if (element1 == "C" and element2 == "CL") or (element1 == "CL" and element2 == "C"):
            distance = 1.790
        if (element1 == "C" and element2 == "BR") or (element1 == "BR" and element2 == "C"):
            distance = 1.910
        if (element1 == "C" and element2 == "I") or (element1 == "I" and element2 == "C"):
            distance = 2.162

        if (element1 == "S" and element2 == "BR") or (element1 == "BR" and element2 == "S"):
            distance = 2.321
        if (element1 == "S" and element2 == "CL") or (element1 == "CL" and element2 == "S"):
            distance = 2.283
        if (element1 == "S" and element2 == "F") or (element1 == "F" and element2 == "S"):
            distance = 1.640
        if (element1 == "S" and element2 == "I") or (element1 == "I" and element2 == "S"):
            distance = 2.687

        if (element1 == "P" and element2 == "BR") or (element1 == "BR" and element2 == "P"):
            distance = 2.366
        if (element1 == "P" and element2 == "CL") or (element1 == "CL" and element2 == "P"):
            distance = 2.008
        if (element1 == "P" and element2 == "F") or (element1 == "F" and element2 == "P"):
            distance = 1.495
        if (element1 == "P" and element2 == "I") or (element1 == "I" and element2 == "P"):
            distance = 2.490
        if (element1 == "P" and element2 == "O") or (element1 == "O" and element2 == "P"):
            distance = 1.6  # estimate based on eye balling Handbook of Chemistry and Physics

        if (element1 == "N" and element2 == "BR") or (element1 == "BR" and element2 == "N"):
            distance = 1.843
        if (element1 == "N" and element2 == "CL") or (element1 == "CL" and element2 == "N"):
            distance = 1.743
        if (element1 == "N" and element2 == "F") or (element1 == "F" and element2 == "N"):
            distance = 1.406
        if (element1 == "N" and element2 == "I") or (element1 == "I" and element2 == "N"):
            distance = 2.2

        if (element1 == "SI" and element2 == "BR") or (element1 == "BR" and element2 == "SI"):
            distance = 2.284
        if (element1 == "SI" and element2 == "CL") or (element1 == "CL" and element2 == "SI"):
            distance = 2.072
        if (element1 == "SI" and element2 == "F") or (element1 == "F" and element2 == "SI"):
            distance = 1.636
        if (element1 == "SI" and element2 == "P") or (element1 == "P" and element2 == "SI"):
            distance = 2.264
        if (element1 == "SI" and element2 == "S") or (element1 == "S" and element2 == "SI"):
            distance = 2.145
        if (element1 == "SI" and element2 == "SI") or (element1 == "SI" and element2 == "SI"):
            distance = 2.359
        if (element1 == "SI" and element2 == "C") or (element1 == "C" and element2 == "SI"):
            distance = 1.888
        if (element1 == "SI" and element2 == "N") or (element1 == "N" and element2 == "SI"):
            distance = 1.743
        if (element1 == "SI" and element2 == "O") or (element1 == "O" and element2 == "SI"):
            distance = 1.631
    ...
    def assign_charges(self):
        ...
        for atom_index in self.NonProteinAtoms:
            ...
            if atom.element == "N":
                ...
                # maybe you only have two hydrogen's added, by they're sp3 hybridized. Just count this as a quartinary amine, since I think the positive charge would be stabalized.
                    ...
                    angle1 = self.functions.angle_between_three_points(
                        atom1.coordinates, nitrogen.coordinates, atom2.coordinates) * 180.0 / math.pi
                    angle2 = self.functions.angle_between_three_points(
                        atom1.coordinates, nitrogen.coordinates, atom3.coordinates) * 180.0 / math.pi
                    angle3 = self.functions.angle_between_three_points(
                        atom2.coordinates, nitrogen.coordinates, atom3.coordinates) * 180.0 / math.pi
            ...
            if atom.element == "C":
                if atom.NumberOfNeighbors() == 3:  # the carbon has only three atoms connected to it
                    nitrogens = self.connected_atoms_of_given_element(
                        atom_index, "N")
                    if len(nitrogens) >= 2:  # so carbon is connected to at least two nitrogens
                        ...
                        if len(nitrogens_to_use) == 2 and not_isolated != -1:
                            ...
                            if (not_isolated_atom.element == "C" and not_isolated_atom.NumberOfNeighbors() == 4) or (not_isolated_atom.element == "O" and not_isolated_atom.NumberOfNeighbors() == 2) or not_isolated_atom.element == "N" or not_isolated_atom.element == "S" or not_isolated_atom.element == "P":
                                ...
                                pt = self.AllAtoms[nitrogens_to_use[0]
                                                   ].coordinates.copy_of()
                                pt.x = pt.x + \
                                    self.AllAtoms[nitrogens_to_use[1]
                                                  ].coordinates.x
                                pt.y = pt.y + \
                                    self.AllAtoms[nitrogens_to_use[1]
                                                  ].coordinates.y
                                ...
                                indexes.extend(self.connected_atoms_of_given_element(
            ...
            if atom.element == "C":  # let's check for a carboxylate
                ...
                if atom.NumberOfNeighbors() == 3:
                    oxygens = self.connected_atoms_of_given_element(
                        atom_index, "O")
                    ...
                    if len(oxygens) == 2:
                        ...
                        if len(self.connected_heavy_atoms(oxygens[0])) == 1 and len(self.connected_heavy_atoms(oxygens[1])) == 1:
                            ...
                            pt = self.AllAtoms[oxygens[0]
                                               ].coordinates.copy_of()
                            pt.x = pt.x + \
                                self.AllAtoms[oxygens[1]].coordinates.x
                            pt.y = pt.y + \
                                self.AllAtoms[oxygens[1]].coordinates.y
                            pt.z = pt.z + \
                                self.AllAtoms[oxygens[1]].coordinates.z
                            pt.x = pt.x / 2.0
                            pt.y = pt.y / 2.0
                            pt.z = pt.z / 2.0
                            chrg = self.charged(
        ...
        for atom_index in self.AllAtoms:
            ...
            if key != curr_res:
                ...
                self.assign_charged_from_protein_process_residue(
                    residue, last_key)
    ...
    def add_aromatic_marker(self, indicies_of_ring):
        ...
        ar_ring = self.aromatic_ring(
            center, indicies_of_ring, [a, b, c, d], radius)
    ...
    def assign_aromatic_rings(self):
        ...
        for ring_index in range(len(AllRings)):
            ...
            for t in range(-3, len(ring)-3):
                ...
                angle = self.functions.dihedral(
                    pt1, pt2, pt3, pt4) * 180 / math.pi
                ...
                for substituent_atom_index in cur_atom.IndeciesOfAtomsConnecting:
                    ...
                    angle = self.functions.dihedral(
                        pt2, pt3, pt4, pt_sub) * 180 / math.pi
                    # 15 degress is the cutoff #, ring[t], ring[t+1], ring[t+2], ring[t+3] # range of this function is -pi to pi
                    if (angle > -165 and angle < -15) or (angle > 15 and angle < 165):
                        is_flat = False
                        break
    ...
    def ring_recursive(self, index, AlreadyCrossed, orig_atom, AllRings):
        ...
        if len(AlreadyCrossed) > 6:
            # since you're only considering aromatic rings containing 5 or 6 members anyway, save yourself some time.
            return
    ...
    def assign_secondary_structure(self):
            ...
            if atom.SideChainOrBackBone() == "BACKBONE":
                if len(atoms) < 8:
                    ...
                    if atoms[0].resid == atoms[1].resid and atoms[0].resid == atoms[2].resid and atoms[0].resid == atoms[3].resid and atoms[0] != atoms[4].resid and atoms[4].resid == atoms[5].resid and atoms[4].resid == atoms[6].resid and atoms[4].resid == atoms[7].resid and atoms[0].resid + 1 == atoms[7].resid and atoms[0].chain == atoms[7].chain:
                        ...
                        phi = self.functions.dihedral(
                            first_C.coordinates, second_N.coordinates, second_CA.coordinates, second_C.coordinates) * 180.0 / math.pi
                        psi = self.functions.dihedral(
                            first_N.coordinates, first_CA.coordinates, first_C.coordinates, second_N.coordinates) * 180.0 / math.pi
                        ...
                        if (phi >= -180 and phi < -40 and psi <= 180 and psi > 90) or (phi >= -180 and phi < -70 and psi <= -165):
                            key1 = str(first_C.resid) + "_" + first_C.chain
                            key2 = str(second_C.resid) + "_" + second_C.chain
                            structure[key1] = "BETA"
                            structure[key2] = "BETA"
        ...
        for atom_index in self.AllAtoms:
            atom = self.AllAtoms[atom_index]
            if atom.residue.strip() in self.protein_resnames and atom.atomname.strip() == "CA":
...
class MathFunctions:
    ...
    def dihedral(self, point1, point2, point3, point4):  # never tested
        ...
        radians = math.atan2(self.dot_product(
            b1XMagb2, b2Xb3), self.dot_product(b1Xb2, b2Xb3))
        return radians

    # As in three connected atoms
    def angle_between_three_points(self, point1, point2, point3):
...
class binana:
    ...
    def __init__(self, ligand_pdbqt_filename, receptor, parameters, line_header, actual_filename_if_ligand_is_list="", actual_filename_if_receptor_is_list=""):
        ...
        for ligand_atom_index in ligand.AllAtoms:
            for receptor_atom_index in receptor.AllAtoms:
                ...
                dist = ligand_atom.coordinates.dist_to(
                ...
                if dist < 2.5:  # less than 2.5 A
                    ...
                    self.hashtable_entry_add_one(
                        ligand_receptor_atom_type_pairs_less_than_two_half, self.list_alphebetize_and_combine(list))
                    ...
                    list = [ligand_atom.atomtype, receptor_atom.atomtype]
                    self.hashtable_entry_add_one(
                        ligand_receptor_atom_type_pairs_less_than_four, self.list_alphebetize_and_combine(list))
                ...
                if dist < 4.0:
                    ...
                    coulomb_energy = (
                        ligand_charge * receptor_charge / dist) * 138.94238460104697e4
                    list = [ligand_atom.atomtype, receptor_atom.atomtype]
                    self.hashtable_entry_add_one(
                ...
                if dist < 4.0:
                    ...
                    if receptor_atom.structure == "ALPHA":
                        pdb_contacts_alpha_helix.AddNewAtom(
                            receptor_atom.copy_of())
                    elif receptor_atom.structure == "BETA":
                        pdb_contacts_beta_sheet.AddNewAtom(
                            receptor_atom.copy_of())
                ...
                if dist < 4.0:
                    ...
                    if ligand_atom.element == "C" and receptor_atom.element == "C":
                        hydrophobic_key = receptor_atom.SideChainOrBackBone() + "_" + \
                            receptor_atom.structure
                        ...
                        for hydrogen in hydrogens:
                            if math.fabs(180 - functions.angle_between_three_points(ligand_atom.coordinates, hydrogen.coordinates, receptor_atom.coordinates) * 180.0 / math.pi) <= 40.0:
                                hbonds_key = "HDONOR_" + hydrogen.comment + "_" + \
                                    receptor_atom.SideChainOrBackBone() + "_" + receptor_atom.structure
        ...
        for ligand_atom_index in ligand.AllAtoms:
            ligand_atom = ligand.AllAtoms[ligand_atom_index]
            self.hashtable_entry_add_one(
                ligand_atom_types, ligand_atom.atomtype)
            ...
            for aromatic2 in receptor.aromatic_rings:
                ...
                if dist < 7.5:  # so there could be some pi-pi interactions.
                    ...
                    aromatic1_norm_vector = point(
                        aromatic1.plane_coeff[0], aromatic1.plane_coeff[1], aromatic1.plane_coeff[2])
                    aromatic2_norm_vector = point(
                        aromatic2.plane_coeff[0], aromatic2.plane_coeff[1], aromatic2.plane_coeff[2])
                    ...
                    if math.fabs(angle_between_planes-0) < 30.0 or math.fabs(angle_between_planes-180) < 30.0:
                        ...
                        for ligand_ring_index in aromatic1.indices:
                            ...
                            pt_on_receptor_plane = self.functions.project_point_onto_plane(
                                ligand.AllAtoms[ligand_ring_index].coordinates, aromatic2.plane_coeff)
                        ...
                        if pi_pi == False:  # if you've already determined it's a pi-pi stacking interaction, no need to keep trying
                            for receptor_ring_index in aromatic2.indices:
                                ...
                                pt_on_ligand_plane = self.functions.project_point_onto_plane(
                                    receptor.AllAtoms[receptor_ring_index].coordinates, aromatic1.plane_coeff)
                    ...
                    # so they're more or less perpendicular, it's probably a pi-edge interaction
                        ...
                        if min_dist <= 5.0:
                            ...
                            if (pt_on_receptor_plane.dist_to(aromatic2.center) <= aromatic2.radius + pi_padding) or (pt_on_lignad_plane.dist_to(aromatic1.center) <= aromatic1.radius + pi_padding):
                                structure = receptor.AllAtoms[aromatic2.indices[0]].structure
                                if structure == "":
                                    structure = "OTHER"  # since it could be interacting with a cofactor or something
        ...
        vina_output = getCommandOutput2(
            parameters.params['vina_executable'] + ' --score_only --receptor ' + receptor_pdbqt_filename + ' --ligand ' + ligand_pdbqt_filename)
        ...
        for item in vina_output:
            item = item.strip()
            if "Affinity" in item:
                vina_affinity = float(item.replace(
                    "Affinity: ", "").replace(" (kcal/mol)", ""))
            if "gauss 1" in item:
        ...
        self.rotateable_bonds_count = {
            'rot_bonds': data['rotateable_bonds_count']}

        self.ligand_receptor_atom_type_pairs_less_than_two_half = {"A_A": 0, "A_C": 0, "A_CL": 0, "A_F": 0, "A_FE": 0, "A_MG": 0, "A_MN": 0, "A_NA": 0, "A_SA": 0, "BR_C": 0, "BR_OA": 0, "C_CL": 0, "CD_OA": 0, "CL_FE": 0, "CL_MG": 0, "CL_N": 0, "CL_OA": 0, "CL_ZN": 0, "C_MN": 0, "C_NA": 0, "F_N": 0, "F_SA": 0, "F_ZN": 0, "HD_MN": 0, "MN_N": 0, "NA_SA": 0, "N_SA": 0, "A_HD": 0, "A_N": 0, "A_OA": 0, "A_ZN": 0, "BR_HD": 0, "C_C": 0,
                                                                   "C_F": 0, "C_HD": 0, "CL_HD": 0, "C_MG": 0, "C_N": 0, "C_OA": 0, "C_SA": 0, "C_ZN": 0, "FE_HD": 0, "FE_N": 0, "FE_OA": 0, "F_HD": 0, "F_OA": 0, "HD_HD": 0, "HD_I": 0, "HD_MG": 0, "HD_N": 0, "HD_NA": 0, "HD_OA": 0, "HD_P": 0, "HD_S": 0, "HD_SA": 0, "HD_ZN": 0, "MG_NA": 0, "MG_OA": 0, "MN_OA": 0, "NA_OA": 0, "NA_ZN": 0, "N_N": 0, "N_NA": 0, "N_OA": 0, "N_ZN": 0, "OA_OA": 0, "OA_SA": 0, "OA_ZN": 0, "SA_ZN": 0, "S_ZN": 0}
        for key in data['ligand_receptor_atom_type_pairs_less_than_two_half']:
            if not key in self.ligand_receptor_atom_type_pairs_less_than_two_half:
                print("\tWARNING: Atoms of types " + key.replace("_",
                      " and ") + " come within 2.5 angstroms of each other.")
                print(
        ...
        for key in data['ligand_receptor_atom_type_pairs_less_than_four']:
            if not key in self.ligand_receptor_atom_type_pairs_less_than_four:
                print("\tWARNING: Atoms of types " + key.replace("_",
                      " and ") + " come within 4 angstroms of each other.")
                print(
                    "\t  The neural networks were not trained to deal with this juxtaposition,")
                print("\t  so it will be ignored.")
                ...
                self.ligand_receptor_atom_type_pairs_less_than_four[key] = data[
                    'ligand_receptor_atom_type_pairs_less_than_four'][key]

        self.ligand_atom_types = {'A': 0, 'BR': 0, 'C': 0, 'CL': 0, 'F': 0,
                                  'HD': 0, 'I': 0, 'N': 0, 'NA': 0, 'OA': 0, 'P': 0, 'S': 0, 'SA': 0}
        for key in data['ligand_atom_types']:
            if not key in self.ligand_atom_types:
                print("\tWARNING: The ligand contains an atoms of type " +
                      key + ". The neural networks")
                print(
        ...
        for key in data['ligand_receptor_atom_type_pairs_electrostatic']:
            if not key in self.ligand_receptor_atom_type_pairs_electrostatic:
                print("\tWARNING: Atoms of types " + key.replace("_",
                      " and ") + ", which come within 4 angstroms of each")
                print(
                ...
                self.ligand_receptor_atom_type_pairs_electrostatic[key] = data[
                    'ligand_receptor_atom_type_pairs_electrostatic'][key]

        self.active_site_flexibility = {'BACKBONE_ALPHA': 0, 'BACKBONE_BETA': 0,
                                        'BACKBONE_OTHER': 0, 'SIDECHAIN_ALPHA': 0, 'SIDECHAIN_BETA': 0, 'SIDECHAIN_OTHER': 0}
        ...
        all = self.active_site_flexibility['BACKBONE_ALPHA'] + self.active_site_flexibility['BACKBONE_BETA'] + self.active_site_flexibility['BACKBONE_OTHER'] + \
            self.active_site_flexibility['SIDECHAIN_ALPHA'] + \
            self.active_site_flexibility['SIDECHAIN_BETA'] + \
            self.active_site_flexibility['SIDECHAIN_OTHER']
        self.active_site_flexibility_all = {'all': all}

        self.hbonds = {'HDONOR-LIGAND_BACKBONE_ALPHA': 0, 'HDONOR-LIGAND_BACKBONE_BETA': 0, 'HDONOR-LIGAND_BACKBONE_OTHER': 0, 'HDONOR-LIGAND_SIDECHAIN_ALPHA': 0, 'HDONOR-LIGAND_SIDECHAIN_BETA': 0, 'HDONOR-LIGAND_SIDECHAIN_OTHER': 0,
                       'HDONOR-RECEPTOR_BACKBONE_ALPHA': 0, 'HDONOR-RECEPTOR_BACKBONE_BETA': 0, 'HDONOR-RECEPTOR_BACKBONE_OTHER': 0, 'HDONOR-RECEPTOR_SIDECHAIN_ALPHA': 0, 'HDONOR-RECEPTOR_SIDECHAIN_BETA': 0, 'HDONOR-RECEPTOR_SIDECHAIN_OTHER': 0}
        for key in data['hbonds']:
            key2 = key.replace("HDONOR_", "HDONOR-")
            self.hbonds[key2] = data['hbonds'][key]

        hdonor_ligand = self.hbonds['HDONOR-LIGAND_BACKBONE_ALPHA'] + self.hbonds['HDONOR-LIGAND_BACKBONE_BETA'] + self.hbonds['HDONOR-LIGAND_BACKBONE_OTHER'] + \
        ...
        hdonor_receptor = self.hbonds['HDONOR-RECEPTOR_BACKBONE_ALPHA'] + self.hbonds['HDONOR-RECEPTOR_BACKBONE_BETA'] + self.hbonds['HDONOR-RECEPTOR_BACKBONE_OTHER'] + \
        ...
        self.hbonds_by_location_of_hdonor = {
            'LIGAND': hdonor_ligand, 'RECEPTOR': hdonor_receptor}

        hbond_backbone = self.hbonds['HDONOR-LIGAND_BACKBONE_ALPHA'] + self.hbonds['HDONOR-LIGAND_BACKBONE_BETA'] + self.hbonds['HDONOR-LIGAND_BACKBONE_OTHER'] + \
        ...
        hbond_sidechain = self.hbonds['HDONOR-LIGAND_SIDECHAIN_ALPHA'] + self.hbonds['HDONOR-LIGAND_SIDECHAIN_BETA'] + self.hbonds['HDONOR-LIGAND_SIDECHAIN_OTHER'] + \
            self.hbonds['HDONOR-RECEPTOR_SIDECHAIN_ALPHA'] + \
            self.hbonds['HDONOR-RECEPTOR_SIDECHAIN_BETA'] + \
            self.hbonds['HDONOR-RECEPTOR_SIDECHAIN_OTHER']
        ...
        hbond_alpha = self.hbonds['HDONOR-LIGAND_BACKBONE_ALPHA'] + self.hbonds['HDONOR-LIGAND_SIDECHAIN_ALPHA'] + \
            self.hbonds['HDONOR-RECEPTOR_BACKBONE_ALPHA'] + \
            self.hbonds['HDONOR-RECEPTOR_SIDECHAIN_ALPHA']
        hbond_beta = self.hbonds['HDONOR-LIGAND_BACKBONE_BETA'] + self.hbonds['HDONOR-LIGAND_SIDECHAIN_BETA'] + \
            self.hbonds['HDONOR-RECEPTOR_BACKBONE_BETA'] + \
            self.hbonds['HDONOR-RECEPTOR_SIDECHAIN_BETA']
        hbond_other = self.hbonds['HDONOR-LIGAND_BACKBONE_OTHER'] + self.hbonds['HDONOR-LIGAND_SIDECHAIN_OTHER'] + \
            self.hbonds['HDONOR-RECEPTOR_BACKBONE_OTHER'] + \
            self.hbonds['HDONOR-RECEPTOR_SIDECHAIN_OTHER']
        self.hbonds_by_structure = {
            'ALPHA': hbond_alpha, 'BETA': hbond_beta, 'OTHER': hbond_other}

        all = self.hbonds['HDONOR-LIGAND_BACKBONE_ALPHA'] + self.hbonds['HDONOR-LIGAND_BACKBONE_BETA'] + self.hbonds['HDONOR-LIGAND_BACKBONE_OTHER'] + self.hbonds['HDONOR-LIGAND_SIDECHAIN_ALPHA'] + self.hbonds['HDONOR-LIGAND_SIDECHAIN_BETA'] + self.hbonds['HDONOR-LIGAND_SIDECHAIN_OTHER'] + \
            self.hbonds['HDONOR-RECEPTOR_BACKBONE_ALPHA'] + self.hbonds['HDONOR-RECEPTOR_BACKBONE_BETA'] + self.hbonds['HDONOR-RECEPTOR_BACKBONE_OTHER'] + \
            self.hbonds['HDONOR-RECEPTOR_SIDECHAIN_ALPHA'] + \
            self.hbonds['HDONOR-RECEPTOR_SIDECHAIN_BETA'] + \
            self.hbonds['HDONOR-RECEPTOR_SIDECHAIN_OTHER']
        self.hbonds_all = {'all': all}

        self.hydrophobics = {'BACKBONE_ALPHA': 0, 'BACKBONE_BETA': 0, 'BACKBONE_OTHER': 0,
        ...
        all = self.hydrophobics['BACKBONE_ALPHA'] + self.hydrophobics['BACKBONE_BETA'] + self.hydrophobics['BACKBONE_OTHER'] + \
            self.hydrophobics['SIDECHAIN_ALPHA'] + \
            self.hydrophobics['SIDECHAIN_BETA'] + \
            self.hydrophobics['SIDECHAIN_OTHER']
        ...
        all = self.stacking['ALPHA'] + \
            self.stacking['BETA'] + self.stacking['OTHER']
        ...
        self.pi_cation = {'LIGAND-CHARGED_ALPHA': 0, 'LIGAND-CHARGED_BETA': 0, 'LIGAND-CHARGED_OTHER': 0,
                          'RECEPTOR-CHARGED_ALPHA': 0, 'RECEPTOR-CHARGED_BETA': 0, 'RECEPTOR-CHARGED_OTHER': 0}
        ...
        self.pi_cation_charge_location = {
            'LIGAND': pi_cation_ligand_charged, 'RECEPTOR': pi_cation_receptor_charged}
        ...
        all = self.pi_cation['LIGAND-CHARGED_ALPHA'] + self.pi_cation['LIGAND-CHARGED_BETA'] + self.pi_cation['LIGAND-CHARGED_OTHER'] + \
            self.pi_cation['RECEPTOR-CHARGED_ALPHA'] + \
            self.pi_cation['RECEPTOR-CHARGED_BETA'] + \
            self.pi_cation['RECEPTOR-CHARGED_OTHER']
        ...
        all = self.t_shaped['ALPHA'] + \
            self.t_shaped['BETA'] + self.t_shaped['OTHER']
        ...
        all = self.salt_bridges['ALPHA'] + \
            self.salt_bridges['BETA'] + self.salt_bridges['OTHER']
...
class command_line_parameters:
    ...
    def __init__(self, parameters):
        ...
        # TRUE by default, but setting to false will speed up execution. Good when rescoring many poses.
...
print(textwrap.fill(
    "NNScore 2.01 is released under the GNU General Public License (see http://www.gnu.org/licenses/gpl.html). If you have any questions, comments, or suggestions, please contact the author, Jacob Durrant, at jdurrant [at] ucsd [dot] edu. If you use NNScore 2.01 in your work, please cite [REFERENCE HERE]."))

print("")

print(textwrap.fill("NNScore 2.01 is based in part on the ffnet python package developed by Marek Wojciechowski (http://ffnet.sourceforge.net/)."))
print("")
...
def calculate_score(lig, rec, cmd_params, actual_filename_if_lig_is_list="", actual_filename_if_rec_is_list="", line_header=""):
    ...
    for net_array in nets:
        try:
            ...
            print(line_header + "Network #" + str(len(scores) + 1) +
                  " gave a score of " + str(round(val, 3)) + " (" + score_to_kd(val) + ")")
            scores.append(val)
            total = total + val
        except OverflowError:
            print(line_header + "The output of network #" + str(len(scores) +
                  1) + " could not be determined because of an overflow error!")
    ...
    if len(scores) != 0:
        ...
        print(line_header + "Best Score:         ",
              round(best_score, 3), "(" + score_to_kd(best_score) + ")")
        print(line_header + "Average Score:      ",
              round(average, 3), "(" + score_to_kd(average) + ")")
...
while len(line) != 0:
    ...
    if line[:6] == "ENDMDL" or len(line) == 0:
        if len(lig_array) != 0 and lig_array != ['']:
            ...
            scores.append([score[0], score[1], score[2],
                          score[3], score[4], model_name])
...
for t in range(20):
    ...
    net_scores = sorted(
        net_scores, key=lambda net_score: net_score[0], reverse=True)  # sort by score
    ...
    for net_score in net_scores:
        print(str(count).center(6, " ") + "|" + net_score[1].center(10, " ") + "|" + str(round(
            net_score[0], 3)).center(7, " ") + "|" + score_to_kd(net_score[0]).replace("Kd = ", "").center(14, " "))
        count = count + 1
    print("")
...
best_scores = sorted(
    scores, key=lambda score: score[2], reverse=True)  # sort by score
...
for score in best_scores:
    print(str(count).center(6, " ") + "|" + score[5].center(10, " ") + "|" + str(round(score[2], 3)).center(
        15, " ") + "|" + score_to_kd(score[2]).replace("Kd = ", "").center(14, " ") + "|" + ("#" + str(score[3])).center(9, " "))
    count = count + 1
...
average_scores = sorted(
    scores, key=lambda score: score[0], reverse=True)  # sort by score
...
for score in average_scores:
    print(str(count).center(6, " ") + "|" + score[5].center(10, " ") + "|" + str(round(score[0], 3)).center(15, " ") + "|" + str(
        round(score[1], 3)).center(20, " ") + "|" + score_to_kd(score[0]).replace("Kd = ", "").center(15, " "))
    count = count + 1
...
for best_network_score in best_network_scores:
    print(textwrap.fill("Best pose scored by network #" + str(count) + ": " + best_network_score[1] + " (Score = " + str(
        round(best_network_score[0], 3)) + " = " + score_to_kd(best_network_score[0]).replace("Kd = ", "") + ")"))
    count = count + 1

print("")

print(textwrap.fill("When the poses were ranked by the best of the 20 network scores associated with each pose, the best-scoring pose was " +
      best_best_score[5] + " (Score = " + str(round(best_best_score[2], 3)) + " = " + score_to_kd(best_best_score[2]).replace("Kd = ", "") + ")"))
print("")
print(textwrap.fill("When the poses were ranked by the average of the 20 network scores associated with each pose, the best-scoring pose was " + best_of_average_scores[5] + " (Score = " + str(round(
    best_of_average_scores[0], 3)) + " +/- " + str(round(best_of_average_scores[1], 3)) + " = " + score_to_kd(best_of_average_scores[0]).replace("Kd = ", "") + "). This is the recommended ranking/scoring metric."))
